...
public class RoboStrokeEventBus extends Thread {
	...
	private static final Logger logger = LoggerFactory.getLogger(com.example.talos_2.RoboStrokeEventBus.class);
	...
	private final LinkedList<com.example.talos_2.BusEventListener> listeners = new LinkedList<com.example.talos_2.BusEventListener>();
		...
		while (!shutdown) {
			try {
				...
				synchronized (listeners) {
					...
					com.example.talos_2.BusEventListener[] list = listeners.toArray(new com.example.talos_2.BusEventListener[listeners.size()]);
					
					for (com.example.talos_2.BusEventListener listener: list) {
						if (shutdown) {
							break;
						}
	 ...
	 */
	public void addBusListener(com.example.talos_2.BusEventListener listener) {
		synchronized (listeners) {
	 ...
	 * @param listener listener object
	 */
	public void removeBusListener(com.example.talos_2.BusEventListener listener) {
		synchronized (listeners) {
