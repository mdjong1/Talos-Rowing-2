...
public class StrokePowerScanner extends StrokeScannerBase implements BusEventListener, ParameterListenerOwner  {
	...
	private final ParameterListenerRegistration[] listenerRegistrations = {
			new ParameterListenerRegistration(com.example.talos_2.ParamKeys.PARAM_STROKE_POWER_AMPLITUDE_FILTER_FACTOR.getId(), new ParameterChangeListener() {
			...
			new ParameterListenerRegistration(com.example.talos_2.ParamKeys.PARAM_STROKE_POWER_MIN_POWER.getId(), new ParameterChangeListener() {
	...
	public StrokePowerScanner(RoboStroke owner, StrokeRateScanner rateScanner) {
		super(owner.getBus(), (Float) owner.getParameters().getValue(com.example.talos_2.ParamKeys.PARAM_STROKE_POWER_AMPLITUDE_FILTER_FACTOR.getId()));
		
		this.params = owner.getParameters();
		this.bus = owner.getBus();
		
		strokePowerTreshold = (Float)params.getValue(com.example.talos_2.ParamKeys.PARAM_STROKE_POWER_MIN_POWER.getId());
	...
	/*	  
	 * @see com.example.talos_2.data.SensorDataFilter#filterData(long, float[])
	 */
	@Override
